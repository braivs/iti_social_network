...
describe("ProfileStatus component", () => {
    ...
    test("after creation <span> should should contains correct status", () => {
        ...
        const root = component.root;
        let span = root.findByType("span")
        expect(span.children[0]).toBe("briws.ru");
    });
    ...
    test("input should be displayed in editMode instead of span", () => {
        ...
        let input = root.findByType("input")
        expect(input.props.value).toBe("briws.ru");
    });

    test("callback should be called", () => {
        const mockCallback = jest.fn();
        const component = create(<ProfileStatus status="briws.ru" updateStatus={mockCallback} />);
        const instance = component.getInstance()
        // @ts-ignore
